...
class WastedTableViewController: UITableViewController {
    ...
    var wasted:[ExpenseMoney] = []
  
    
    func getWastedObject() -> [ExpenseMoney] {
        let realm = try! Realm()
        let wobjects = realm.objects(ExpenseMoney.self).filter("iswastedmoney == true")
        return Array<ExpenseMoney>(wobjects)
    }
