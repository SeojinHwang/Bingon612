...
class WishlistTableViewController: UITableViewController {
    ...
    @IBAction  func unwindFromWishModalVC (segue: UIStoryboardSegue) {
        ...
        let newwish = Wish()
        let realm = try! Realm ()
        let result = realm.objects(WastedMoney.self)
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell{
        ...
        if indexPath.section == 0 {
            ...
            let realm = try! Realm ()
            let waste = realm.objects(WastedMoney.self)
            let wastedamount = waste.last!
            cell.fixedLabel1.text = layoutItem.fixedLabel1
            cell.wastedLAbel.text = "\(wastedamount.wastedmoney)"
